...
def validate_xml_file_by_resource_type(request, resource_type):
    ...
    try:
        ...
        if not xml_matching_resource_type_validation_details['is_root_tag_valid']:
            ...
            raise InvalidRootElementNameForMetadataFileException(f"Expected the metadata file to have a root element name of \"{xml_matching_resource_type_validation_details['expected_root_tag']}\", but got \"{xml_matching_resource_type_validation_details['root_tag']}\".")
        ...
        unregistered_references = validation.get_unregistered_references_from_xml(xml_file_parsed)
        ...
        if len(unregistered_referenced_resource_hrefs) > 0:
            ...
            err_class = UnregisteredMetadataDocumentException
        ...
        if len(unregistered_referenced_ontology_term_hrefs) > 0:
            raise UnregisteredOntologyTermException('Unregistered ontology term IRIs: %s.' % ', '.join(unregistered_referenced_ontology_term_hrefs))
